...
class AutonomousAgent:
    ...
    def calculate_score(self, m):
        # Defensive checks for missing or invalid metrics
        try:
            trades = m.get('trades', 0)
            if trades <= 5:
                return -100.0
            
            profit_pct = m.get('profit_pct', 0.0)
            ...
            score = profit_pct * profit_factor * math.log10(max(trades, 1) + 1)
            return float(score)
        except Exception as e:
            self.log(f"Score calculation error: {e} with metrics: {m}", "ERROR")
            return -100.0
